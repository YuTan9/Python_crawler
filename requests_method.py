...
for i in range(0, len(data_list)):
    ...
    try: 
        ...
        temp = str(result.content).replace("null", str("test"))
        if(eval(result.content).get('data')):
            allJournals = eval(result.content).get('data')
            for journal in allJournals:
                if(journal.get('citingJournal') in data_list): # the journal is in the journals we have
                    # print(journal.get('citingJournal'))
    ...
    try:
        ...
        allJournals = eval(result.content).get('data')
        for journal in allJournals:
            if(journal.get('citedJournal') in data_list): # the journal is in the journals we have
...
newFile.save('Alter_connection.xls')
...
r = requests.get('http://jcr.incites.thomsonreuters.com.libproxy1.nus.edu.sg/CitedJournalDataJson.action',
    headers=headers, params=params)
test = eval(r.content)
print(test.get('data')[0])
"""